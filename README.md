...
**Docker:** `docker compose up` starts MySQL and the web dashboard. A public tunnel URL is printed in the logs. For a **stable URL** that works from any network (e.g. when away from your machine), add `NGROK_AUTHTOKEN` to `.env` (free at [ngrok.com](https://ngrok.com)); the same URL will persist across restarts. Evolution can run in Docker: `docker compose --profile evolution run --rm evolution python scripts/run_verse_evolution.py --theme "pressure,mask" --population 30 --generations 5`. See [Remote Workflow](docs/REMOTE_WORKFLOW.md) for SSH-based dev. The dashboard and MySQL run in containers; evolution scripts (`run_verse_qd.py`, `run_verse_evolution.py`, etc.) are usually run on the **host** with the same DB env (`RAPBOT_USE_DB=1`, `RAPBOT_DB_HOST=localhost`, etc.) so runs appear in the dashboard. Use the host’s Python environment (e.g. `pip install -e ".[full]"`) for evolution; the web container is kept lightweight and does not run evolution.

**Docker-only (no host Python/DB):** When the host cannot reach MySQL or lacks Python deps, run everything in the evolution container. Policy updates (`update_learned_policy.py`) and convergence analysis must run in Docker so they can reach the DB. Bootstrap the learned policy before continuous runs: `docker compose --profile evolution run --rm evolution python scripts/update_learned_policy.py --force --limit 300`. Then run continuous evolution: `docker compose --profile evolution run --rm evolution python scripts/run_continuous.py --no-docker --bootstrap-policy --config config/continuous_runs.yaml` (use `--no-docker` so evolution runs inline in the container instead of spawning nested Docker).
...
- **`run_continuous.py`** — Continuous loop over YAML configs; `--config`, `--bootstrap-policy`, `--bootstrap-if-empty`, `--update-policy-every`, `--elite-replay-fraction`, `--parallel N` (max runs in flight, default 1). When using Docker, policy updates run inside the evolution container so DB is reachable.
